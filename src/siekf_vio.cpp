...
map<int, double> SiekfVio::chi_squared_test_table;

SiekfVio::SiekfVio(ros::NodeHandle& pnh):
...
void SiekfVio::initializeGravityAndBias() {
...
void SiekfVio::mocapOdomCallback(
...
void SiekfVio::batchImuProcessing(const double& time_bound) {
...
void SiekfVio::processModel(const double& time,
...
void SiekfVio::predictNewState(const double& dt,
    const Vector3d& gyro,
    const Vector3d& acc) {
...
void SiekfVio::stateAugmentation(const double& time) {
...
void SiekfVio::addFeatureObservations(
    const CameraMeasurementConstPtr& msg) {
...
void SiekfVio::measurementJacobian(
...
void SiekfVio::featureJacobian(
  ...
  for (const auto& cam_id : valid_cam_state_ids) {
    ...
    if(cam_state_cntr>0){
        H_xj.block<4, 3>(stack_cntr, 21) = -H_xi.block<4,3>(0,0);
    }
...
void SiekfVio::measurementUpdate(
...
void SiekfVio::removeLostFeatures() {
...
void SiekfVio::findRedundantCamStates(
...
void SiekfVio::pruneCamStateBuffer() {
...
void SiekfVio::publish(const ros::Time& time) {
...
Matrix<double,6,6> SiekfVio::convertPoseCov(){
